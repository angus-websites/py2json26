...
with open("composer.json", "r") as file:
    ...
    pr_version = composer.get("version")

if not pr_version:
    print("⚠️  composer.json has no version field")
    sys.exit(1)
...
try:
    git_show = subprocess.run(
        ["git", "show", "origin/main:composer.json"],
        check=True,
        text=True,
        capture_output=True,
    )
    main_composer = json.loads(git_show.stdout)
    main_version = main_composer.get("version")
except subprocess.CalledProcessError:
    print("⚠️  No composer.json found on origin/main. Skipping version comparison.")
    sys.exit(0)
